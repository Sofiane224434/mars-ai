      ...
      <div className={`flex-1 ${isJuryVariant ? 'space-y-4' : 'overflow-y-auto pr-2 custom-scrollbar space-y-5'}`}>
        ...
        {isJuryVariant && (
          <div className="rounded-xl border border-bleu-canard/30 bg-bleu-canard/10 px-3 py-2 text-xs text-bleu-ciel md:text-sm">
            Pas besoin d&apos;insérer de lien ni de token : le lien de connexion sécurisé, le token d&apos;accès personnel et la date d&apos;expiration seront générés puis ajoutés automatiquement à la fin de l&apos;email après envoi. Le prénom du juré sera également personnalisé automatiquement.
          </div>
        )}
